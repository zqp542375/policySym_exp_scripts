...
do
	...
	for ((batch=1; batch<=total_batches; batch++)); do
		...
		# Start containers for the current batch
			...
			echo ""
			echo "start Docker container mythril_${group_index}_${run_idx} for contract group ${contract_group_name_prefix}_${group_index}" 
	
	
			docker run --rm --cpus 4 -v ${root_dir}:${container_work_dir} --name mythril_${group_index}_${run_idx}  --entrypoint ${run_script} ${image_id} ${container_work_dir} ${dataset_name} ${group_index} ${contract_info_csv_name_prefix} ${contract_group_name_prefix} ${cli_timeout} ${tool_timeout} ${run_idx} &
