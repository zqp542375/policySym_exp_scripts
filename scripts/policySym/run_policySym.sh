...
while IFS="," read solidity_name solc_version contract_name 
 ...
 echo "++++ ${solidity_name}  :  ${solc_version}  :  ${contract_name} ++++"  | tee -a ${result_dir}${solidity_name}__${contract_name}_${tool_name}_${tool_timeout}_${run_idx}.txt
	...
	start=$(date +%s.%N)
	timeout ${cli_timeout} semyth analyze ${contract_dir}${solidity_folder}/${solidity_name}:${contract_name} -fdg -fss rl_mlp_policy --create-timeout 60 --execution-timeout ${tool_timeout} --solver-timeout ${solver_timeout} --pruning-factor 1.0 --consider-all-reads 0 --preprocess-timeout 100 --execution-times-limit 5 --optimization 1  | tee -a ${result_dir}${solidity_name}__${contract_name}_${tool_name}_${tool_timeout}_${run_idx}.txt
